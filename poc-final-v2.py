...
def log_pre_processing(logger: logging.Logger, log_lines: list[str]) -> list[list[str]]:
    ...
    logger.info('End log pre-processing: {0} - Lines pre-processed: {1}'.format(str(time.time() - tracking_time), str(num_lines_inserted)))
    tracking_time = time.time()
...
"""
def obs_filtering(logger: logging.Logger, log_lines: list[list[str]], obs_list: list[list[str]]) -> list[list[str]]:
    global tracking_time
    ...
    for log_section in log_lines:
        ...
        if(section_passed_flag):
            ...
            logger.info('Section within successful observation timeframe found: {0} - obs_tpl_time: {1}'.format(str(time.time() - tracking_time), respective_tpl_time))
            tracking_time = time.time()
            ...
            for _, line in log_section:
                ...
                img_line_2 = re.search(r'([0-2][0-9]:[0-5][0-9]:[0-5][0-9](.[0-9]+)*)> TEL ACTO (INTTIME)', line)

                if(force_line is not None):
                    logger.info('{0} deleted by obs_filtering'.format(force_line.group()))
                
                elif(img_line is not None):
                    logger.info('{0} deleted by obs_filtering'.format(img_line.group()))

                elif(img_line_2 is not None):
                    logger.info('{0} deleted by obs_filtering'.format(img_line_2.group()))

    ### Checkpoint - End observation log lines filtering
    logger.info('End observation log lines filtering: {0} - Lines passed: {1}'.format(str(time.time() - tracking_time), str(num_lines_passed)))
    tracking_time = time.time()
...
"""
def new_obs_filtering(logger: logging.Logger, log_lines: list[str], obs_list: list[list[str]], date: str) -> dict:
    global tracking_time

    ### Checkpoint - Start new observation log lines filtering
    logger.info('Start new observation log lines filtering: {0}'.format(str(time.time() - tracking_time)))
    tracking_time = time.time()
    ...
    for tpl_instrument, tpl_date, exptime in obs_list:
        ...
        obs_time_intervals[tpl_instrument].append([datetime.strptime(tpl_date, "%Y-%m-%dT%H:%M:%S") - timedelta(seconds=lower_margin), datetime.strptime(tpl_date, "%Y-%m-%dT%H:%M:%S") + timedelta(seconds=float(exptime) + upper_margin)])
    ...
    for instrument in obs_time_intervals.keys().__iter__():
        ...
        for line in log_lines:        
            date_format = re.search(r'([A-Za-z]{3} [0-9]+ [0-2][0-9]:[0-5][0-9]:[0-5][0-9](.[0-9]+)*)', line)
...
"""
def log_parsing_ttp(logger: logging.Logger, log_sections: list[list[str]], templates_list: list[str]) -> list[dict]:
    global tracking_time
...
Returns:
    A list of dictionaries, with each containing the dynamic data extracted from a log line.
"""
def log_parsing_regex(logger: logging.Logger, log_sections: dict, templates_list: list[str]) -> list[dict]:
    global tracking_time
    ...
    for log_section_key in log_sections.keys().__iter__():
        for line in log_sections[log_section_key]:
            ...
            if(parsing_flag):
                ...
                logger.info('Log line parsing succesful: {0}'.format(str(time.time() - tracking_time)))
                tracking_time = time.time()
                ...
                if(force_line is not None):
                    logger.info('{0} deleted by log_parsing_regex'.format(force_line.group()))
                
                elif(img_line is not None):
                    logger.info('{0} deleted by log_parsing_regex'.format(img_line.group()))

                elif(img_line_2 is not None):
                    logger.info('{0} deleted by log_parsing_regex'.format(img_line_2.group()))


    ### Checkpoint - End log line parsing
    logger.info('End log line parsing: {0} - Lines parsed: {1}'.format(str(time.time() - tracking_time), str(num_lines_parsed)))
    tracking_time = time.time()
...
"""
def generate_dataframes(logger: logging.Logger, parsed_data: list[dict[str, str]]) -> list[pd.DataFrame]:
    global tracking_time
        ...
        if(line["group"] == "FORCES"):
            ...
            if(line["label"] == "INTTIME"):
                ...
                if(image_index == -1):
                    ...
                    if(
                        dict_images["integration_time"][image_index] is not None and
                        img_date <= datetime.strptime(line["time"], "%H:%M:%S").time() <= (datetime.combine(buffer_min_day, img_date) + timedelta(seconds=acceptance_threshold)).time()
                    ):
...
"""
def link_images(logger: logging.Logger, df_images: pd.DataFrame, img_folder: str) -> pd.DataFrame:
    global tracking_time
...
"""
def validate_forces(logger: logging.Logger, df_f_dist: pd.DataFrame, df_images: pd.DataFrame) -> pd.DataFrame:
    global tracking_time
    ...
    while(f_dist_index < len(nu_df_f_dist['id_f_dist'])):
        ...
        for img_times_index in range(len(img_times_buffer) - 1):
            ...
            if(curr_time < nu_df_f_dist['timestamp'][f_dist_index] and nu_df_f_dist['timestamp'][f_dist_index] < next_time):
                remove_flag = False
        ...
        if(remove_flag):
            ...
            nu_df_f_dist = nu_df_f_dist[nu_df_f_dist['id_f_dist'] != f_dist_remove_index]
...
"""
def link_dataframes(logger: logging.Logger, df_corrections: pd.DataFrame, df_attr: pd.DataFrame, name_attr: str, time_field: str) -> pd.DataFrame:
    global tracking_time
...
Returns:
    None
"""
def save_report(log_lines: list[str], mid_files_list: list[list[any]], file_name: str) -> None:
    # Denormalizing lists
    ...
    with open(file_name, "w") as reporte:
        ...
        reporte.write("Number of AO correction instance log lines parsed: " + str(num_corr_lines) + "\n")
        reporte.write("Number of force re-distribution instance log lines parsed: " + str(num_forces_lines) + "\n")
        reporte.write("Number of image obtention instance log lines parsed: " + str(num_img_lines) + "\n")
...
def parse_args(args_list: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument("date", type=str,
        help="Date to analyzed")
    
    parser.add_argument("ut", type=str,
        help="Number of the UT telescope to be analyzed")  
    ...
    parser.add_argument("-o", "--obsprocess",
        action="store_false",
    ...
    parser.add_argument("-i", "--linkimages",
        action="store_false",
    ...
    parser.add_argument("-l", "--linkdataframes",
        action="store_false",
    ...
    parser.add_argument("-s", "--save",
        type=str,
        help="Stores resulting dataframes in csv files")
...
def process_path(logger: logging.Logger, args: argparse.Namespace) -> None:
    ...
    df_list, mid_list = dataframe_generation_subpath(logger=logger, 
                                           log_lines=log_lines, 
                                           date=args.date, 
    ...
    refined_df_list = dataframe_refining_subpath(logger=logger, 
                               df_list=df_list,
                               img_linking=args.linkimages,
                               df_linking=args.linkdataframes)
                               

    dataframe_showcasing_subpath(logger=logger,
                                log_file=log_lines,
...
"""
def dataframe_generation_subpath(logger: logging.Logger, log_lines: list[str], date: str, tplt_arch_flag: str, pre_process_flag: bool, obs_process_flag: bool) -> list[list[any]]:
...
"""
def dataframe_refining_subpath(logger: logging.Logger, df_list: list[pd.DataFrame], img_linking: bool, df_linking: bool) -> list[pd.DataFrame]:
...
Returns:
    None
"""
def dataframe_showcasing_subpath(logger: logging.Logger, log_file: list[str], mid_files_list: list[list[str]], df_list: list[pd.DataFrame], console_flag: bool, save_flag: str, report_flag: str) -> None:
    #### Denormalizing dataframe list
    ...
    #### Print dataframes in console
    if report_flag:
        save_report(log_file, mid_files_list, 'Reporte {0}'.format(datetime.now().strftime("%Y-%m-%d_%H-%M-%S")))
